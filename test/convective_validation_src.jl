...
function validate_mass_addition(arch; boundary_layer, output = false)
    ...
    q_stencil = CenterField(
        grid,
        Float64;
        indices = (-halo_indices:halo_indices, -halo_indices:halo_indices, :),
    )
    ...
    parameters = (;
        ...
        τ_c,
        h_c,
        nghosts = numelements_to_traverse - 1,
        radiative_cooling_rate,
        q0 = heating_amplitude,
        R = convective_radius,
        relaxation_parameter,
        relaxation_height,
        Δx2 = grid_spacing_x^2,
        Δy2 = grid_spacing_y^2,
        heating_stencil = q_stencil,
        boundary_layer,
    )
    ...
    function progress(sim)
        ...
        @info(
            @sprintf(
                "Iter: %d, time: %.1f, Δt: %.1f, max|h|: %.2f, min|h|: %.2f",
                m.clock.iteration,
                m.clock.time,
                sim.Δt,
                maximum(m.solution.h),
                minimum(m.solution.h)
            )
        )
    end
