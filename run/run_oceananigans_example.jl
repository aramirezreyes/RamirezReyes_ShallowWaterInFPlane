...
isconvecting = OffsetArray(zeros(Bool,Nx + 2numelements_to_traverse_x, Ny + 2numelements_to_traverse_y),
                           (-numelements_to_traverse_x + 1) : (Nx + numelements_to_traverse_x),
                           (-numelements_to_traverse_y + 1) : (Ny + numelements_to_traverse_y)) 
convection_triggered_time = OffsetArray(zeros(Nx + 2numelements_to_traverse_x, Ny + 2numelements_to_traverse_y),
                                        (-numelements_to_traverse_x + 1) : (Nx + numelements_to_traverse_x),
                                        (-numelements_to_traverse_y + 1) : (Ny + numelements_to_traverse_y)) 
