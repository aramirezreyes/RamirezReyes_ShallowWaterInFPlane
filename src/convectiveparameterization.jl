...
"""
function update_convective_events!(
    architecture::CPU,
    isconvecting,
...
function update_convective_events!(
    ...
    kernel = @cuda launch = false update_convective_events_gpu!(
        isconvecting,
        convection_triggered_time,
        h,
        t,
...
Yang, D., and A. P. Ingersoll, 2013: Triggered Convection, Gravity Waves, and the MJO: A Shallow-Water Model. J. Atmos. Sci., 70, 2476–2486, https://doi.org/10.1175/JAS-D-12-0255.1.
"""
@inline function update_convective_events_cpu!(
    isconvecting,
    convection_triggered_time,
    h,
    t,
    ...
    @tturbo for ind in eachindex(int_h)
        ...
        will_start_convecting =
            needs_to_convect_by_height * iszero(needs_to_convect_by_time) #time needs be updated?

        needs_to_convect = needs_to_convect_by_time | needs_to_convect_by_height
        int_isconvecting[ind] = needs_to_convect
        int_convection_triggered_time[ind] =
            ifelse(will_start_convecting, t, int_convection_triggered_time[ind])
    end
    return nothing
end
...
Yang, D., and A. P. Ingersoll, 2013: Triggered Convection, Gravity Waves, and the MJO: A Shallow-Water Model. J. Atmos. Sci., 70, 2476–2486, https://doi.org/10.1175/JAS-D-12-0255.1.
"""
function update_convective_events_gpu!(
    isconvecting,
    convection_triggered_time,
    h,
    t,
    ...
    @inbounds for i = index_x:stride_x:Nx
        for j = index_y:stride_y:Ny
...
@inline function heat_at_point(
    ...
    @inbounds for neigh_j = nn:np
        jinner = j + neigh_j
        @inbounds for neigh_i = nn:np
            iinner = i + neigh_i
            if isconvecting[iinner, jinner]
                forcing = add_or_substract_mass(
                    forcing,
                    heat(
                        current_time,
                        convection_triggered_time[iinner, jinner],
                        τc,
                        heating_stencil[neigh_i, neigh_j],
                    ),
                )
            end
        end
...
function fill_heating_stencil_gpu!(q, q0, Δx, R2)
    ...
    @inbounds for i = (index_x-1):stride_x:Nx
        for j = (index_y-1):stride_y:Ny
            if (i^2 + j^2) * Δx^2 <= R2
...
function model_forcing(i, j, k, grid, clock, model_fields, parameters)
    @inbounds target_height =
        isnothing(parameters.relaxation_height) ? parameters.mean_h[1] :
        parameters.relaxation_height
    boundary_layer = parameters.boundary_layer
    large_scale_forcing =
        boundary_layer ? parameters.large_scale_forcing :
        -1 * parameters.large_scale_forcing
    @inbounds heat_at_point(
        i,
        j,
        k,
        clock.time,
        ...
        parameters.nghosts,
        parameters.heating_stencil,
        parameters.boundary_layer,
    ) + large_scale_forcing -
              (model_fields.h[i, j, k] - target_height) * parameters.relaxation_parameter
end
...
@kernel function model_forcing_only_convec!(convec_heating, clock, parameters)
    ...
    @inbounds convec_heating[i, j, k] = heat_at_point(
        i,
        j,
        k,
        clock.time,
