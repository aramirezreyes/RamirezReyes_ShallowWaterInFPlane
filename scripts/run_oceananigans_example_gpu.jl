...
grid = RectilinearGrid(size = (Nx, Ny),
                            x = (0, Lx), y = (0, Ly),
                              ...
                              halo = (max(numelements_to_traverse_x,3),
                                      max(numelements_to_traverse_y,3)))
...
#build forcing
convec_forcing = Forcing(model_forcing,discrete_form=true,
                         parameters=(;architecture = architecture, τ_c = τ_c,h_c = h_c,
                                     isconvecting = isconvecting,
                                     convection_triggered_time = convection_triggered_time,
                                     R = convective_radius, q0 = heating_amplitude,
                                     radiative_cooling_rate = radiative_cooling_rate,
...
function h̄(x, y, z)
    rand() > 0.5 ? Lz  : 39.0#-  Δη * exp(-((x-Lx/2)^2/(0.1*Lx)^2 + (y-Ly/2)^2/(0.1*Ly)^2 ))
...
function progress(sim)
    ...
    #@info "Go run update_...!"
    m = sim.model
    update_convective_events!(m.architecture,p.isconvecting,p.convection_triggered_time,
                              p.h.data,m.clock.time,p.τ_c,p.h_c,m.grid.Nx,
                              m.grid.Ny, p.nghosts_x,p.nghosts_y)
...
simulation = Simulation(model, Δt = 1, stop_time = 86400, progress = progress,
                        parameters = (h = h, isconvecting = isconvecting,
                                      convection_triggered_time = convection_triggered_time,
                                      τ_c, h_c, nghosts_x = numelements_to_traverse_x,
                                      nghosts_y = numelements_to_traverse_y ))
